...
void CHudDeathNotice::Paint()
{
	...
	for ( int i = 0; i < iCount; i++ )
	{
		...
		if (m_DeathNotices[i].Killer.iEntIndex < 0)
			iKillerTeam = -1*m_DeathNotices[i].Killer.iEntIndex;
		...
		Color iconColor( 120, 120, 120, 255 );

		// Draw death weapon
		//If we're using a font char, this will ignore iconTall and iconWide
		icon->DrawSelf( x, y-6, iconWide, iconTall, iconColor );
		x += iconWide;		
...
void CHudDeathNotice::FireGameEvent( IGameEvent * event )
{
	...
	{
		const char *champion;
		if (!strcmp( fullkilledwith, "death_cap_vamp" ))
		{
			champion = "Vampires";
			killer = -3;
		}
		...
		Q_snprintf( sDeathMsg, sizeof( sDeathMsg ), "%s capped point %s.\n", deathMsg.Killer.szName, missionconfig );
	...
	{
		//BB: custom death notification messages...
		if (!strcmp( fullkilledwith, "death_stake" ))
		{
		...
		else if (!strcmp( fullkilledwith, "death_gorebar" ))
		{
			Q_snprintf( sDeathMsg, sizeof( sDeathMsg ), "%s clawed %s's eyes out.\n", deathMsg.Killer.szName, deathMsg.Victim.szName );
		}
		else if (!strcmp( fullkilledwith, "death_degenbar" ))
		{
			Q_snprintf( sDeathMsg, sizeof( sDeathMsg ), "%s clawed %s's eyes out.\n", deathMsg.Killer.szName, deathMsg.Victim.szName );
		}
		else if (!strcmp( fullkilledwith, "death_grenade_BB" ))
		{
			Q_snprintf( sDeathMsg, sizeof( sDeathMsg ), "%s retched all over %s.\n", deathMsg.Killer.szName, deathMsg.Victim.szName );
		}
		else if (!strcmp( fullkilledwith, "death_leap" ))
		{
			Q_snprintf( sDeathMsg, sizeof( sDeathMsg ), "%s mauled %s.\n", deathMsg.Killer.szName, deathMsg.Victim.szName );
		}
		else if (!strcmp( fullkilledwith, "death_blood" ))
		{
			Q_snprintf( sDeathMsg, sizeof( sDeathMsg ), "%s succumbed to %s's blood.\n", deathMsg.Victim.szName, deathMsg.Killer.szName  );
		}
		else if (!strcmp( fullkilledwith, "death_goreexp" ))
		{
			Q_snprintf( sDeathMsg, sizeof( sDeathMsg ), "%s got too close to %s.\n", deathMsg.Victim.szName, deathMsg.Killer.szName  );
		}
		else if (!strcmp( fullkilledwith, "death_degen" ))
		{
			Q_snprintf( sDeathMsg, sizeof( sDeathMsg ), "%s scared %s to death.\n", deathMsg.Killer.szName, deathMsg.Victim.szName  );
		}
		else
		{
