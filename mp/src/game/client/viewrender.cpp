...
Color CViewRender::HSL_RGB(int H, int S, int L)
{
	int temp[3];
	S /= 255.0;
	L /= 255.0;
	H *= 6;
	int range = H % 255;
	int phase = floor((float)H / 255.0);
	int mid = int(7.0 - phase) % 3;
	temp[(int)floor(float(phase + 7.0)/2.0)%3] = 255;
	temp[(int)(floor((float)phase/2.0)+5.0)%3] = 0;
	if ((phase%2) == 1)
		temp[mid] = 255.0-range;
	else
		temp[mid] = range;

	for (int i = 0; i<3; i++)
	{
		temp[i] = 255.0 - ((255.0 - temp[i]) * S);
		temp[i] *= L;
	}
...
void CViewRender::DrawMissionIndicator()
{
	//BB: render stuff here? ON TOP OF BRUSHES...
	//CBasePlayer::GetLocalPlayer()->SetEFlags( EFL_DIRTY_ABSTRANSFORM );

	//BB: easy way out only draw for mission type capture zone...
	//return;
	...
	if (!(temp2.x == 0 && temp2.y == 0 && temp2.z == 0))
	{
		...
		Vector temp = CBasePlayer::GetLocalPlayer()->EyePosition()-temp2;
		//Vector forward;
		//CBasePlayer::GetLocalPlayer()->EyeVectors(&forward);
		int dist = temp.Length();
		/*if (dist == 0) //no dividy by zero
			dist = 1;*/

		/*Vector color;
		Color tmp_clr = HSL_RGB(HL2MPRules()->capture_color, 255, 255);
		color.x = tmp_clr.r()/255.0;
		color.y = tmp_clr.g()/255.0;
		color.z = tmp_clr.b()/255.0;
		int defending_team = 3;
		int viewdist = 500;
		if (((C_BaseHLPlayer *)CBasePlayer::GetLocalPlayer())->m_HL2Local.covenflags & COVEN_HAS_FARSIGHT || ((C_BaseHLPlayer *)CBasePlayer::GetLocalPlayer())->m_HL2Local.covenflags & COVEN_AURA_WISDOM)
			viewdist = 1800;
		else if (((C_BaseHLPlayer *)CBasePlayer::GetLocalPlayer())->m_HL2Local.covenflags & COVEN_HAS_MINOR_SIGHT)
			viewdist = 800;
		if (((C_BaseHLPlayer *)CBasePlayer::GetLocalPlayer())->m_HL2Local.covenflags & COVEN_SLAYERS_DEF)
			defending_team = 2;

		if (CBasePlayer::GetLocalPlayer()->GetTeamNumber() != defending_team && !(((C_BaseHLPlayer *)CBasePlayer::GetLocalPlayer())->m_HL2Local.covenflags & COVEN_MISSION_RET) && dist < viewdist)//1500 is too far
		{
			VectorNormalize(temp);
			CBasePlayer::GetLocalPlayer()->CreateObjectiveCircle();
			CBasePlayer::GetLocalPlayer()->m_objectiveCircle->m_FXData.SetMaterial("effects/objective_marker");
			CBasePlayer::GetLocalPlayer()->m_objectiveCircle->m_FXData.SetNormal( temp );
			//CBasePlayer::GetLocalPlayer()->m_objectiveCircle->m_FXData.m_flStartScale = 1600.0f/dist;
			CBasePlayer::GetLocalPlayer()->m_objectiveCircle->m_FXData.m_flStartScale = 50.0f;
			//CBasePlayer::GetLocalPlayer()->m_objectiveCircle->Update(CBasePlayer::GetLocalPlayer()->EyePosition() - 20*temp, Vector(1,0,0),true);
			CBasePlayer::GetLocalPlayer()->m_objectiveCircle->Update(temp2, color, true);
		}
		else if (CBasePlayer::GetLocalPlayer()->GetTeamNumber() == defending_team)
		{
			//VectorNormalize(temp);
			CBasePlayer::GetLocalPlayer()->CreateObjectiveCircle();
			CBasePlayer::GetLocalPlayer()->m_objectiveCircle->m_FXData.SetNormal( temp );
			CBasePlayer::GetLocalPlayer()->m_objectiveCircle->m_FXData.SetMaterial("effects/objective_marker_def");
			//CBasePlayer::GetLocalPlayer()->m_objectiveCircle->m_FXData.m_flStartScale = 1600.0f/dist;
			CBasePlayer::GetLocalPlayer()->m_objectiveCircle->m_FXData.m_flStartScale = 50.0f;
			//CBasePlayer::GetLocalPlayer()->m_objectiveCircle->Update(CBasePlayer::GetLocalPlayer()->EyePosition() - 20*temp, Vector(1,0,0),true);
			CBasePlayer::GetLocalPlayer()->m_objectiveCircle->Update(temp2, color, true);
