			...
			if (IsPlayer())
			{
				...
				else if (GetTeamNumber() == COVEN_TEAMID_SLAYERS)
				{
					...
					if (myServerRagdoll)
					{
						...
						((CBaseAnimating *)myServerRagdoll)->AddGlowEffect(false, true, false, false, false, true, 3000.0f);
					}
				}
...
<<<<<<< HEAD:mp/src/game/server/basecombatcharacter.cpp
...
>>>>>>> upstream/master:src/game/server/basecombatcharacter.cpp
	}
